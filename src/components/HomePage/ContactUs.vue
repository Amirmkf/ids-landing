<template>
  <v-parallax :src="parallaxImage" id="contact-us" height="550">
    <section class="pb-8 contact-us-color">
      <v-container fluid>
        <v-row justify="center" class="">
          <v-col cols="12" sm="5">
            <h1 class="font-weight-light display-1">{{ $t("contactUs.title") }}</h1>
            <h3 class="font-weight-light mt-3">
              {{ $t("contactUs.text") }}
            </h3>
            <h3 class="font-weight-light mt-5">
              <svg class="v-icon address-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                   viewBox="0 0 24 24" fill="none">
                <path
                    d="M12 13.4299C13.7231 13.4299 15.12 12.0331 15.12 10.3099C15.12 8.58681 13.7231 7.18994 12 7.18994C10.2769 7.18994 8.88 8.58681 8.88 10.3099C8.88 12.0331 10.2769 13.4299 12 13.4299Z"
                    stroke="currentColor" stroke-width="1.5"/>
                <path
                    d="M3.62001 8.49C5.59001 -0.169998 18.42 -0.159997 20.38 8.5C21.53 13.58 18.37 17.88 15.6 20.54C13.59 22.48 10.41 22.48 8.39001 20.54C5.63001 17.88 2.47001 13.57 3.62001 8.49Z"
                    stroke="currentColor" stroke-width="1.5"/>
              </svg>
              <span class="font-weight-bold">
                {{ $t("contactUs.address") }}:
              </span>
              <p>{{ $t("contactUs.firstAddress") }}
                <br>
                {{ $t("contactUs.secondAddress") }}</p>
            </h3>
            <h3 class="font-weight-light mt-4">
              <svg class="v-icon phone-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                   viewBox="0 0 24 24" fill="none">
                <path
                    d="M21.97 18.33C21.97 18.69 21.89 19.06 21.72 19.42C21.55 19.78 21.33 20.12 21.04 20.44C20.55 20.98 20.01 21.37 19.4 21.62C18.8 21.87 18.15 22 17.45 22C16.43 22 15.34 21.76 14.19 21.27C13.04 20.78 11.89 20.12 10.75 19.29C9.6 18.45 8.51 17.52 7.47 16.49C6.44 15.45 5.51 14.36 4.68 13.22C3.86 12.08 3.2 10.94 2.72 9.81C2.24 8.67 2 7.58 2 6.54C2 5.86 2.12 5.21 2.36 4.61C2.6 4 2.98 3.44 3.51 2.94C4.15 2.31 4.85 2 5.59 2C5.87 2 6.15 2.06 6.4 2.18C6.66 2.3 6.89 2.48 7.07 2.74L9.39 6.01C9.57 6.26 9.7 6.49 9.79 6.71C9.88 6.92 9.93 7.13 9.93 7.32C9.93 7.56 9.86 7.8 9.72 8.03C9.59 8.26 9.4 8.5 9.16 8.74L8.4 9.53C8.29 9.64 8.24 9.77 8.24 9.93C8.24 10.01 8.25 10.08 8.27 10.16C8.3 10.24 8.33 10.3 8.35 10.36C8.53 10.69 8.84 11.12 9.28 11.64C9.73 12.16 10.21 12.69 10.73 13.22C11.27 13.75 11.79 14.24 12.32 14.69C12.84 15.13 13.27 15.43 13.61 15.61C13.66 15.63 13.72 15.66 13.79 15.69C13.87 15.72 13.95 15.73 14.04 15.73C14.21 15.73 14.34 15.67 14.45 15.56L15.21 14.81C15.46 14.56 15.7 14.37 15.93 14.25C16.16 14.11 16.39 14.04 16.64 14.04C16.83 14.04 17.03 14.08 17.25 14.17C17.47 14.26 17.7 14.39 17.95 14.56L21.26 16.91C21.52 17.09 21.7 17.3 21.81 17.55C21.91 17.8 21.97 18.05 21.97 18.33Z"
                    stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10"/>
              </svg>
              <span class="font-weight-bold">
                {{ $t("contactUs.phone") }}:
              </span>
              {{ $t("contactUs.phoneText") }}
            </h3>
            <h3 class="font-weight-light mt-4">
              <svg class="v-icon email-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                   viewBox="0 0 24 24" fill="none">
                <path
                    d="M17 20.5H7C4 20.5 2 19 2 15.5V8.5C2 5 4 3.5 7 3.5H17C20 3.5 22 5 22 8.5V15.5C22 19 20 20.5 17 20.5Z"
                    stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round"
                    stroke-linejoin="round"/>
                <path d="M17 9L13.87 11.5C12.84 12.32 11.15 12.32 10.12 11.5L7 9" stroke="currentColor"
                      stroke-width="1.5"
                      stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
              <span class="font-weight-bold">
                {{ $t("contactUs.email") }}:
              </span>
              {{ $t("contactUs.emailText") }}
            </h3>
          </v-col>
          <v-col cols="12" sm="7">
            <v-form ref="form" v-model="valid" :lazy-validation="lazy">
              <div v-if="this.$store.state.locale === 'EN'">
                <v-text-field v-model="name" :rules="nameRules" label="Name" required></v-text-field>
                <v-text-field v-model="email" :rules="emailRules" label="E-mail" required></v-text-field>
                <v-textarea v-model="textArea" :rules="textAreaRules" label="Message" required/>
              </div>
              <div v-else>
                <v-text-field v-model="name" :rules="nameRules" reverse
                              label="نام" required></v-text-field>
                <v-text-field v-model="email" :rules="emailRules" reverse
                              label="ایمیل" required></v-text-field>
                <v-textarea v-model="textArea" :rules="textAreaRules" reverse
                            label="پیام" required/>
              </div>

              <v-btn :disabled="!valid" color="primary" :dark="valid" rounded block class="mt-3" @click="submit">
                {{ $t("contactUs.send") }}
              </v-btn>
            </v-form>
          </v-col>
        </v-row>
      </v-container>
      <div class="svg-border-waves text-white">
        <v-img src="~@/assets/img/borderWavesBlue.svg"/>
      </div>
      <v-snackbar v-model="snackbar.enabled" timeout="3000" right top :color="snackbar.color">
        {{ snackbar.text }}

        <template v-slot:action="{ attrs }">
          <v-btn text v-bind="attrs" @click="snackbar.enabled = false">
            Close
          </v-btn>
        </template>
      </v-snackbar>
    </section>
  </v-parallax>
</template>

<script>
// import {db} from '@/main'

export default {
  data: () => ({
    icons: ["fa-facebook", "fa-twitter", "fa-linkedin", "fa-instagram"],
    valid: true,
    name: "",
    email: "",
    textArea: "",
    lazy: false,
    snackbar: {
      enabled: false,
      text: '',
      color: ''
    },
    parallaxImage: require('@/assets/images/4436009_93190.jpg')
  }),
  computed: {
    nameRules() {
      if (this.$store.state.locale === "EN") {
        return [
          (v) => !!v || "The name field is required",
          (v) => (v && v.length >= 6) || "The name must be longer than 6 characters",
        ]
      } else {
        return [
          (v) => !!v || "فیلد نام الزامی است",
          (v) => (v && v.length >= 6) || "نام باید بیشتر از 6 کاراکتر باشد",
        ]
      }
    },
    emailRules() {
      if (this.$store.state.locale === "EN") {
        return [
          (v) => !!v || "The email field is required",
          (v) => /.+@.+\..+/.test(v) || "Email must be valid",
        ]
      } else {
        return [
          (v) => !!v || "فیلد ایمیل الزامی است",
          (v) => /.+@.+\..+/.test(v) || "ایمیل باید معتبر باشد",
        ]
      }
    },
    textAreaRules() {
      if (this.$store.state.locale === "EN") {
        return [
          (v) => !!v || "The text field is required",
          (v) => (v && v.length >= 10) || "Minimum 10 characters",
        ]
      } else {
        return [
          (v) => !!v || "فیلد متن الزامی است",
          (v) => (v && v.length >= 10) || "حداقل 10 کاراکتر",
        ]
      }
    }
  },
  methods: {
    submit() {
      /*db.collection("contactData").add({
        name: this.name,
        email: this.email,
        message: this.textArea
      }).then(() => {
        this.snackbar.text = "Mensagem enviada com sucesso"
        this.snackbar.color = "success"
        this.snackbar.enabled = true
      }).catch(() => {
        this.snackbar.text = "Erro ao enviar mensagem"
        this.snackbar.color = "danger"
        this.snackbar.enabled = true
      })*/
    }
  }
};
</script>